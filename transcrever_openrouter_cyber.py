...
# ============================================================
# GLOSSÁRIO E REGRAS (tradução) — CyberSec geral / Linux / Windows / Redes
# ============================================================
GLOSSARIO_CYBERSEC = (
    "GLOSSÁRIO — NÃO traduza os termos abaixo (mantenha em inglês, por "
    "serem de uso consagrado em cursos/certificações de cibersegurança, "
    "administração Linux/Windows e redes no Brasil, como CompTIA "
    "Security+/Network+/Linux+, CCNA, CEH, GCIH, GCFA, ITIL e cursos de "
    "SOC/Blue Team):\n"
    "- Segurança geral / SOC: malware, ransomware, phishing, spear "
    "phishing, zero-day, exploit, payload, backdoor, Indicator of "
    "Compromise (IOC), Indicator of Attack (IOA), threat hunting, threat "
    "intelligence, SIEM, SOC, EDR, XDR, IDS, IPS, firewall, DLP, MFA, 2FA, "
    "PAM, patch management, hardening, CVE, CVSS, pentest, red team, blue "
    "team, purple team, incident response, playbook, SOAR, brute force, "
    "wordlist\n"
    "- Windows: Active Directory (AD), Domain Controller, Group Policy "
    "Object (GPO), Kerberos, NTLM, LDAP, PowerShell, Registry, Event "
    "Viewer, WMI, RDP, SMB, UAC, BitLocker, Sysinternals, Task Scheduler, "
    "service account\n"
    "- Linux: kernel, shell, bash, systemd, cron, crontab, iptables, "
    "nftables, SELinux, AppArmor, syslog, daemon, sudo, root, chmod, "
    "chown, symlink, mount, filesystem, package manager\n"
    "- Redes: TCP/IP, DNS, DHCP, VLAN, VPN, NAT, subnet, gateway, proxy, "
    "load balancer, routing, switching, BGP, OSPF, ARP, ICMP, port "
    "scanning, sniffing, MTU, QoS, ACL\n"
    "- Siglas técnicas em geral: HTTP, HTTPS, SSH, FTP, SSL, TLS, API, "
    "JSON, XML, etc.\n"
    "Termos do dia a dia de TI com tradução consolidada (arquivo, "
    "servidor, usuário, senha, navegador, rede, endereço) DEVEM ser "
    "traduzidos normalmente. Na dúvida, prefira manter em inglês se for o "
    "uso comum em cursos como CompTIA/CCNA/CEH."
)

# Versão curta pra usar como "context" na transcrição (bias de vocabulário).
# Isso é EXPERIMENTAL: o campo de contexto do Qwen3-ASR-Flash existe na API
# nativa da DashScope, mas não há confirmação de que o OpenRouter repassa
# esse campo corretamente pro provider. O script testa no primeiro chunk e
# desliga sozinho se der erro (ver USAR_CONTEXTO_TRANSCRICAO abaixo).
CONTEXTO_ASR_CYBERSEC = (
    "malware, ransomware, phishing, exploit, payload, backdoor, IOC, IOA, "
    "SIEM, SOC, EDR, IDS, IPS, firewall, MFA, CVE, red team, blue team, "
    "incident response, Active Directory, Domain Controller, GPO, "
    "Kerberos, NTLM, LDAP, PowerShell, RDP, SMB, kernel, bash, systemd, "
    "iptables, SELinux, sudo, TCP/IP, DNS, DHCP, VLAN, VPN, NAT, gateway, "
    "routing, switching, BGP, ARP"
)
